...
class TestLsProcessors(TestBase):
    ...
    def test_ls_processors(self, mock_get):
        responses_add = [
            PROCESSOR_SUMMARY_RESP,
            PROCESSOR_MEMBERS_RESP,
    ...
    @patch("aiohttp.ClientSession.get")
    def test_ls_processors_summary_proc_data_error(self, mock_get):
        responses_add = [
            PROCESSOR_SUMMARY_RESP_FAULTY,
        ]
        responses = INIT_RESP + responses_add
    ...
    @patch("aiohttp.ClientSession.get")
    def test_ls_processors_summary_value_error(self, mock_get):
        responses_add = [
            "",
        ]
        responses = INIT_RESP + responses_add
    ...
    @patch("aiohttp.ClientSession.get")
    def test_ls_processors_details_not_found(self, mock_get):
        responses_add = [
            PROCESSOR_SUMMARY_RESP,
            "Not Found"
        ]
    ...
    @patch("aiohttp.ClientSession.get")
    def test_ls_processors_details_value_error(self, mock_get):
        responses_add = [
            PROCESSOR_SUMMARY_RESP,
            ""
        ]
